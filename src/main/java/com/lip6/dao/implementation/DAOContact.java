...
public class DAOContact implements IDAOContact {
  ...
  public List<Contact> getAllInformationsAboutContactByHisEmail(String email) {
    String request =
        "SELECT DISTINCT c FROM Contact c LEFT JOIN FETCH c.phones WHERE c.email=:mail";
    String request2 =
        "SELECT DISTINCT c FROM Contact c LEFT JOIN FETCH c.contactGroups WHERE c in :getFullInformationOfContactByEmail";

    EntityManager entityManager = this.emf.createEntityManager();

    List<Contact> getFullInformationOfContactByEmail =
        entityManager.createQuery(request, Contact.class)
            .setParameter("mail", email).setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getResultList();
  ...
  public int getNumberOfContactByEmail(String email) {
    ...
    TypedQuery<Long> numberOfContactByEmailQuery =
        entityManager.createNamedQuery("Contact.CountContactByEmail",
            Long.class);
  ...
  public ContactDTO getContact(long id) {
    ...
    String getDTOContactByIdRequest =
        "SELECT NEW com.lip6.domain.model.DTO.ContactDTO(c.idContact, c.email, c.lastName, c.firstName) from Contact c where c.idContact= :id";
  ...
  public int updateContact(Contact contact) {
    ...
    tx.begin();
    String request =
        "UPDATE Contact c SET c.firstName=:firstName, c.lastName=:lastName, c.email= :email WHERE c.idContact=:idContact";
  ...
  public boolean deleteContact(long id) {
    ...
    if (contact != null) {
      ...
      contact.getContactGroups().forEach($ -> {
        $.getContacts().remove(contact);
        entityManager.merge($);
      });
      contact.getPhones().removeAll(contact.getPhones());
