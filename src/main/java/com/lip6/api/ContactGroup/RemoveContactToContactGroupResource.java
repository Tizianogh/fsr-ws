...
/**
 * Servlet implementation class RemoveContactToContactGroupResource
 */
@WebServlet("/cg/remove")
public class RemoveContactToContactGroupResource extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (idGroup == null || idGroup.isEmpty()) {
      ...
      Optional<ContactGroup> retrievedRemoveContactGroup =
          this.contactGroupService.removeContactToContactGroup(
              createdContact,
              idGroupModifiedToLong);
      if (!retrievedRemoveContactGroup.isPresent()) {
        responseBuilder = new Response.Builder().with($ -> {
          $.message = String.format(
        ...
        responseBuilder = new Response.Builder().with($ -> {
          $.message = String.format(
              "[%s] - Completed - Le contact a bien été supprimé du groupe.", new Date());
          $.timeStamp = new Date();
          $.data = new AbstractMap.SimpleEntry("result", retrievedRemoveContactGroup.get());
